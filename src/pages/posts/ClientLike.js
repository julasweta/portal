...
function ClientLike() {
  ...
  const changeCategory = (id, category) => {
    const checkRecipes = recipes.filter(
      (item) => item.category + item.id === category + id
    );
    const checkUsefuls = usefuls.filter(
      (item) => item.category + item.id === category + id
    );
              ...
              <div className="card-body">
                ...
                <Link
                  onClick={() =>
                    onActiveName(
                      post.id,
                      changeCategory(post.id, post.category),
                      post.category
                    )
                  }
                  to={`/${changeCategory(post.id, post.category)}/${
                    post.category
                  }/${post.id}`}
                >
                  {" "}
